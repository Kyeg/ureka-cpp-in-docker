...
TEST(TestFlow8, AttakerForgeToApplyUticket) {
    ...
    generated_request3["holder_id"] = cloud_server_atk.shared_data->this_person.person_pub_key;
    generated_request3["u_ticket_type"] = u_ticket::TYPE_SELFACCESS_UTICKET;
    generated_request3["task_scope"] = generated_task_scope;

    cloud_server_atk.flow_issuer_issue_u_ticket->issuer_issue_u_ticket_to_herself(target_device_id, generated_request3.dump());
    ...
    iot_device.msg_receiver->recv_xxx_message();
    cloud_server_atk.msg_receiver->recv_xxx_message(); // -> already failed
    /* iot_device.msg_receiver->recv_xxx_message();
    cloud_server_atk.msg_receiver->recv_xxx_message(); */

    bool res1 = iot_device.shared_data->result_message.find("-> FAILURE: VERIFY_HOLDER_ID") != std::string::npos;
