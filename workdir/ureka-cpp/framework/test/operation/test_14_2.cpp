...
TEST(TestFlow8, AttakerInterceptAndPreemptToApplyUticket) {
    ...
    string intercepted_rticket_json = user_agent.shared_data->device_table[target_device_id].device_r_ticket_for_owner;

    cloud_server_atk.shared_data->device_table[target_device_id] = OtherDevice(target_device_id, intercepted_uticket_json, intercepted_rticket_json, 2);
    ...
    target_device_id  = iot_device.shared_data->this_device.device_pub_key;
    intercepted_uticket_json = user_agent.shared_data->device_table[target_device_id].device_u_ticket_for_owner;
    cloud_server_atk.generated_msg_storer->store_generated_xxx_u_ticket(intercepted_uticket_json);

    cloud_server_atk.executor->self_execute_update_ticket_order("holder-generate-or-receive-uticket", 1, uticket_from_json_str(intercepted_uticket_json), RTicket());

    string generated_command = "HELLO-1";
    cloud_server_atk.flow_apply_u_ticket->holder_apply_u_ticket(target_device_id, generated_command);
    iot_device.msg_receiver->recv_xxx_message();
    ...
    cout << "iot_device.shared_data->result_message: " << iot_device.shared_data->result_message << endl;
    cout << "cloud_server_atk.shared_data->result_message: " << cloud_server_atk.shared_data->result_message << endl;
    bool res1 = iot_device.shared_data->result_message.find("-> FAILURE: SELF_VERIFY_DEVICE_SIGNATURE_ON_R_TICKET") != std::string::npos;
