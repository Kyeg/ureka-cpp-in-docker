...
TEST(TestFlow16, TestFailWhenApplyWrongTaskScope) {
    ...
    // assert "FAILURE" in iot_device received message
    ASSERT_TRUE(iot_device.shared_data->result_message.find("FAILURE") != string::npos);
    ASSERT_NE(iot_device.shared_data->current_session.plaintext_cmd, generated_command);
    ASSERT_NE(iot_device.shared_data->current_session.plaintext_data, "DATA: " + generated_command);
    ...
    cloud_server_ep.flow_issue_u_token->holder_send_cmd(target_device_id, generated_command, true);
    ...
    // assert "SUCCESS" in iot_device received message
